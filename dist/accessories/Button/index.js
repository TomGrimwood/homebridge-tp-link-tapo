    ...
    constructor(hub, platform, accessory, log, deviceInfo) {
        ...
        const checkStatus = async () => {
            ...
            try {
                ...
                if (!response) {
                    this.log.debug('[%s] button poll: no response, backing off', this.mac);
                ...
                this.log.debug('[%s] button poll failed: %s', this.mac, error instanceof Error ? error.message : String(error));
                await (0, delay_1.default)(500);
            }
            checkStatus();
        };
